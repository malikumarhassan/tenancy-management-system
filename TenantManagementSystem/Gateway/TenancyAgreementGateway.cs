    ...
    public class TenancyAgreementGateway : Gateway
    {
        ...
        public int Update(TenancyAgreement aTenancyAgreement)
        {
            ...
            try
            {
               ...
               " CancellationDate = @cancellationDate, CompletedDate = @completedDate, Remarks = @remarks, ChequeNumber = @chequeNumber, ChequeAmount = @chequeAmount, ChequeDate = @chequeDate " +
               " WHERE Id = @id";
                ...
                Command.Parameters.AddWithValue("cancellationDate", aTenancyAgreement.CancellationDate);
                Command.Parameters.AddWithValue("completedDate", aTenancyAgreement.CompletedDate);
                Command.Parameters.AddWithValue("remarks", aTenancyAgreement.Remarks);
                Command.Parameters.AddWithValue("chequeNumber", aTenancyAgreement.ChequeNumber);
                Command.Parameters.AddWithValue("chequeAmount", aTenancyAgreement.ChequeAmount);
                Command.Parameters.AddWithValue("chequeDate", aTenancyAgreement.ChequeDate);
        ...
        public List<TenancyAgreement> GetAllTenancyAgreement()
        {
            ...
            try
            {
                Query = " SELECT TA.Id, TA.AgreementNumber, TA.LessorId, LSR.Name LessorName,  TA.LesseeId, LSE.Name LesseeName, " +
                        ...
                        " TA.CreatedBy, TA.CreatedDate, TA.UpdatedBy, TA.UpdatedDate, TA.CancellationDate, TA.CompletedDate,  TA.Remarks, TA.ChequeNumber, TA.ChequeAmount, TA.ChequeDate " +
                ...
                while (Reader.Read())
                {
                    TenancyAgreement aTenancyAgreement = new TenancyAgreement()
                    {
                        ...
                        CancellationDate = string.IsNullOrEmpty(Convert.ToString(Reader["CancellationDate"])) ? DateTime.Now : Convert.ToDateTime(Reader["CancellationDate"]),
                        CompletedDate = string.IsNullOrEmpty(Convert.ToString(Reader["CompletedDate"])) ? DateTime.Now : Convert.ToDateTime(Reader["CompletedDate"]),
                        ChequeDate = string.IsNullOrEmpty(Convert.ToString(Reader["ChequeDate"])) ? DateTime.Now : Convert.ToDateTime(Reader["ChequeDate"]),
                        Remarks = Convert.ToString(Reader["Remarks"]),
                        ChequeNumber = Convert.ToString(Reader["ChequeNumber"]),
                        ChequeAmount = string.IsNullOrEmpty(Convert.ToString(Reader["ChequeAmount"])) ? 0 : Convert.ToDecimal(Reader["ChequeAmount"])
            ...
            try
            {
                ...
                while (Reader.Read())
                {
                    TenancyAgreement aTenancyAgreement = new TenancyAgreement()
                    {
                        ...
                        IsCompleted = string.IsNullOrEmpty(Convert.ToString(Reader["IsCompleted"])) ? false : Convert.ToBoolean(Reader["IsCompleted"]),
                        IsCancelled = string.IsNullOrEmpty(Convert.ToString(Reader["IsCancelled"])) ? false : Convert.ToBoolean(Reader["IsCancelled"]),
                        CompletedDateDetail = Convert.ToString(Reader["CompletedDateDetail"]),
                        CancellationDateDetail = Convert.ToString(Reader["CancellationDateDetail"]),
